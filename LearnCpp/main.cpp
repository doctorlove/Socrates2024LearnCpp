...
int input(std::istream& is)
{
    ...
    if (!is)
    {
        is.clear(); // clear the fail flag (should mop up too - see next function)
...
{
    static std::uniform_int_distribution dist{ 0, 2 }; // warning some distribs have state, so keep this hanging around
